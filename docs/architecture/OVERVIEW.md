...
- **Flag `__E2E_AWAIT_AUDIT__`:** forma canónica de probar el audit trail end-to-end. Inyectar en `beforeEach` vía `page.addInitScript(() => { (window as any).__E2E_AWAIT_AUDIT__ = true })`. Hace que `makeAuditable` espere el INSERT antes de devolver el control al test, evitando cancelación por `afterEach`. En PROD el flag no está definido → fire-and-forget sin cambio de comportamiento. Ver ADR-026 §"Modo awaitable en E2E".
