...
// Dos secciones en scroll vertical:
//   1. Contexto del cliente (campos del perfil)
//   2. Registro de fricciones y oportunidades
//
// Estado gestionado por Zustand (useCompanyProfileStore).
    ...
    <div className="relative">
      <select
        ...
        className={[
          ...
          'bg-gray-900 dark:bg-gray-900 border border-white/8',
          'text-gray-100 transition-colors duration-150',
          'focus:outline-none focus:border-navy/60 focus:ring-1 focus:ring-navy/30',
          !value ? 'text-gray-500' : 'text-gray-100',
        ].join(' ')}
      >
      ...
      <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-500"
    ...
    <button
      ...
      className={[
        ...
        selected
          ...
          : 'bg-gray-800 text-gray-400 border-white/8 hover:border-navy/40 hover:text-gray-200',
      ].join(' ')}
    >
      {label}
    </button>
    ...
    <button
      ...
      className={[
        ...
        selected
          ...
          : 'bg-gray-800 text-gray-400 border-white/8 hover:border-white/20',
      ].join(' ')}
    >
      {label}
    </button>
...
function FrictionCard({
  ...
  return (
    <div className="rounded-xl bg-gray-800/60 border border-white/6 p-5 space-y-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="h-6 w-6 rounded-full bg-navy/20 border border-navy/30 flex items-center justify-center">
            <span className="text-[10px] font-bold text-navy-dark dark:text-info-soft">{index + 1}</span>
          </div>
          <span className="text-xs font-semibold text-gray-300">Fricción / Oportunidad</span>
        </div>
        <button
          onClick={onRemove}
          className="h-6 w-6 rounded flex items-center justify-center text-gray-600 hover:text-danger-dark hover:bg-danger-light/10 transition-colors"
      ...
      {/* Tipo de problema + Área funcional */}
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full px-3 py-2 rounded-lg text-xs bg-gray-900 border border-white/8 text-gray-200 placeholder-gray-600 resize-none focus:outline-none focus:border-navy/60 focus:ring-1 focus:ring-navy/30 transition-colors"
          />
        </div>
      </div>
    </div>
...
export function CompanyProfileView() {
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <div className="sticky top-[57px] z-10 bg-gray-950/95 backdrop-blur-sm border-b border-white/6 px-8 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">

          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg bg-navy/20 border border-navy/30 flex items-center justify-center">
                <svg width="13" height="13" viewBox="0 0 14 14" fill="none" stroke="#9BB5D9" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              {savedFlash ? (
                ...
                <>
                  <svg width="12" height="12" viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    ...
                    <path d="M9 2v4H5V2" />
                    <rect x="4" y="8" width="6" height="5" rx="0.5" />
                  </svg>
                  Guardar contexto
                </>
              )}
      ...
      <div className="max-w-5xl mx-auto px-8 py-8 space-y-8">
        ...
        <div className="rounded-2xl bg-gray-900 border border-white/6 p-6 space-y-6">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-lg text-sm bg-gray-900 border border-white/8 text-gray-100 placeholder-gray-600 focus:outline-none focus:border-navy/60 focus:ring-1 focus:ring-navy/30 transition-colors"
            />
          </div>

          {/* Grid 2×2 — selects principales */}
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <LeanSelect
                value={profile.tamanoEmpresa}
                onChange={(v) => updateField('tamanoEmpresa', v)}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-2.5 rounded-lg text-sm bg-gray-900 border border-white/8 text-gray-100 placeholder-gray-600 resize-none focus:outline-none focus:border-navy/60 focus:ring-1 focus:ring-navy/30 transition-colors"
              />
            </div>
          </div>

          {/* Áreas prioritarias — chips */}
          <div>
        ...
        <div className="rounded-2xl bg-gray-900 border border-white/6 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <SectionLabel>Fricciones y oportunidades detectadas</SectionLabel>
              <p className="text-xs text-gray-500 -mt-1">
                Registra los problemas e ineficiencias detectados durante las entrevistas.
                Estos datos alimentan T4 (priorización) y T6 (governance).
              </p>
            </div>
            {profile.fricciones.length > 0 && (
              <span className="text-[10px] font-mono text-gray-500 shrink-0 ml-4">
                {profile.fricciones.length}/{profile.fricciones.length} registros
              </span>
            )}
          </div>
          ...
          {profile.fricciones.length > 0 ? (
            ...
            <div className="rounded-xl border border-dashed border-white/8 py-8 text-center">
              <p className="text-xs text-gray-600">
                No hay fricciones registradas todavía.
              </p>
              <p className="text-[10px] text-gray-700 mt-1">
                Las fricciones se registran durante las entrevistas de diagnóstico.
              </p>
            </div>
          )}

          {/* Botón añadir fricción */}
          <button
            onClick={addFriction}
            className="w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-dashed border-white/10 text-xs text-gray-500 hover:border-navy/40 hover:text-gray-300 hover:bg-navy/5 transition-all duration-150"
          >
          ...
          {profile.fricciones.length >= 2 && (
            <div className="rounded-xl bg-gray-800/50 border border-white/4 p-4 mt-2">
              ...
              <div className="flex gap-6 text-xs">
                <div>
                  ...
                  <span className="ml-2 font-semibold text-danger-dark">
                    {profile.fricciones.filter((f) => f.frecuencia === 'Alta').length}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Alto impacto</span>
                  <span className="ml-2 font-semibold text-warning-dark">
                    {profile.fricciones.filter((f) => f.impacto === 'Alto').length}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Registradas</span>
                  <span className="ml-2 font-semibold text-gray-300">
                    {profile.fricciones.length}
                  </span>
                </div>
                <div>
                  <span className="text-gray-500">Sin completar</span>
                  <span className="ml-2 font-semibold text-gray-500">
                    {profile.fricciones.filter((f) => !f.tipo || !f.frecuencia || !f.impacto).length}
                  </span>
                </div>
              </div>
            </div>
