// ============================================================
// Transformación Digital — Dimensiones y subdimensiones
//
// Equivalente a T1_MaturityRadar/constants.ts para el dominio
// Transformación Digital (6 dimensiones × 4 subdimensiones = 24 puntos).
//
// Tipos importados desde ../constants.ts — misma estructura que AI Adoption.
// Suma de pesos = 1.0 (verificada en test)
// ============================================================
...
export const TD_DIMENSION_DEFINITIONS: DimensionDefinition[] = [D1, D2, D3, D4, D5, D6]

/** Mapa de acceso rápido por código de dimensión (TD) */
export const TD_DIMENSION_MAP = Object.fromEntries(
  TD_DIMENSION_DEFINITIONS.map((d) => [d.code, d])
) as Record<string, DimensionDefinition>

/** Mapa de acceso rápido por código de subdimensión (TD) */
export const TD_SUBDIMENSION_MAP = Object.fromEntries(
  TD_DIMENSION_DEFINITIONS.flatMap((d) =>
    d.subdimensions.map((s) => [s.code, s])
  )
) as Record<string, SubdimensionDefinition>

/** Total de subdimensiones = 24 */
export const TD_TOTAL_SUBDIMENSIONS = TD_DIMENSION_DEFINITIONS.reduce(
