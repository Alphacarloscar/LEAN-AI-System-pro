...
export function ArchetypeMessagesTab({ messages }: { messages: ArchetypeMessage[] }) {
  ...
  const DEPT_CFG_LOCAL: Record<string, { badgeBg: string; badgeText: string; fill: string }> = {
    adoptador:    { fill: '#5FAF8A', badgeBg: 'bg-success-light', badgeText: 'text-success-dark' },
    ambassador:   { fill: '#6A90C0', badgeBg: 'bg-info-light',    badgeText: 'text-info-dark'    },
    decisor:      { fill: '#2A2822', badgeBg: 'bg-warm-100',      badgeText: 'text-warm-700'     },
    reticente:    { fill: '#C8860A', badgeBg: 'bg-warning-light', badgeText: 'text-warning-dark' },
    critico:      { fill: '#C06060', badgeBg: 'bg-danger-light',  badgeText: 'text-danger-dark'  },
  }
            ...
            <button
              ...
              className={[
                ...
                selected === m.archetypeCode
                  ...
                  : 'border-border dark:border-white/6 text-text-muted hover:border-navy/20 bg-white dark:bg-warm-900',
              ].join(' ')}
            >
              <span
      ...
      {msg && (
        <div className="flex-1 min-w-0 space-y-4">
          ...
          <Card variant="outlined" padding="none" className="rounded-xl p-5">
            ...
            <p className="text-base font-semibold text-lean-black dark:text-warm-50 leading-snug">
              "{msg.headline}"
            </p>
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-4">
              <Card variant="flat" padding="none" className="rounded-xl border border-danger-light bg-danger-light/20 p-4">
                <p className="text-[10px] font-mono uppercase tracking-widest text-danger-dark mb-2 flex items-center gap-1"><AlertTriangle size={11} strokeWidth={1.5} /> No decir</p>
          ...
          <Card variant="flat" padding="none" className="rounded-xl border border-border dark:border-white/6 bg-warm-50 dark:bg-warm-800/50 p-4 flex items-start gap-3">
            <Lightbulb size={16} strokeWidth={1.5} className="flex-shrink-0 text-text-subtle" />
            <div>
